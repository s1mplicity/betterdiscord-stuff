    ...
    return [
      {
        title: 'Fixed',
        type: 'fixed',
        items: ['Plugin works once more.', 'Menus should no longer cause your discord to hard crash.', 'Fixed context menus not working at all.']
      }
    ];
  }
  initialize() {
  ...
  patchContextMenus() {
    ...
    Patcher.after(
      ...
      (_, [props], ret) => {
        ...
        menu.push(XenoLib.createContextMenuGroup([XenoLib.createContextMenuSubMenu(this.settings.contextmenuSubmenuName, newItems, 'mlv2')]));
      }
    );
    Patcher.after(
      ...
      (_, [props], ret) => {
        ...
        if (!Array.isArray(menu)) return;
        const addElement = (label, callback, id, options = {}) => newItems.push(XenoLib.createContextMenuItem(label, callback, id, options));
        addElement('Open Logs', () => this.openWindow(), 'open');
    ...
    const handleWhiteBlackList = (newItems, id) => {
      ...
      addElement(
        ...
        'change-notif-blacklist'
      );
    };

    Patcher.after(
      WebpackModules.find(({ default: defaul }) => defaul.displayName === 'ChannelListTextChannelContextMenu' && defaul.toString().search(/\(0,\w\.default\)\(\w,\w\),\w=\(0,\w\.default\)\(\w,\w\),\w=\(0,\w\.default\)\(\w,\w\),\w=\(0,\w\.default\)\(\w\),\w=\(0,\w\.default\)\(\w\.id\)/) !== -1),
      'default',
      (_, [props], ret) => {
        const newItems = [];
        const menu = ZeresPluginLibrary.Utilities.getNestedProp(
        ...
        addElement(
          ...
          () => {
            this.menu.filter = `channel:${props.channelId}`;
            this.openWindow();
          },
          'open-channel'
        );
    ...
    Patcher.after(
      WebpackModules.find(({ default: { displayName } }) => displayName === 'GuildChannelUserContextMenu'),
      'default',
      (_, [props], ret) => {
        const newItems = [];
        const menu = ZeresPluginLibrary.Utilities.getNestedProp(
    ...
    Patcher.after(
      WebpackModules.find(({ default: { displayName } }) => displayName === 'DMUserContextMenu'),
      'default',
      (_, [props], ret) => {
        const newItems = [];
        const menu = ZeresPluginLibrary.Utilities.getNestedProp(
    ...
    Patcher.after(
      WebpackModules.find(({ default: defaul }) => defaul.displayName === 'GroupDMContextMenu'),
      'default',
      (_, [props], ret) => {
        const newItems = [];
        const menu = ZeresPluginLibrary.Utilities.getNestedProp(
